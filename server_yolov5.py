...
from drone_control_yolov5 import get_controller, get_lastest_frame
...
# Endpoint để cập nhật vùng phát hiện "person in water" từ drone
# (giữ luôn route cũ /update_aruco_markers để không phá cấu trúc cũ)
@app.route('/update_person_zones', methods=['POST'])
...
def update_person_zones():
    ...
    try:
        ...
        # New: zones. Old: markers.
        zones = payload.get('zones')
        if zones is None:
            zones = payload.get('markers', {})

        if not isinstance(zones, dict):
            return jsonify({'error': 'zones must be a dict'}), 400

        # Merge into global store (reuse aruco_markers var name for compatibility)
        for k, v in zones.items():
            aruco_markers[str(k)] = v

        # Emit both new + old event names for web compatibility
        socketio.emit('person_zones_update', {'zones': aruco_markers})
        socketio.emit('aruco_markers_update', {'markers': aruco_markers})

        return jsonify({'status': 'success', 'zones_received': len(zones)})
    except Exception as e:
        print(f"Error updating person zones: {e}")
        return jsonify({'error': str(e)}), 500

# Endpoint để lấy danh sách ArUco markers hiện tại
# Endpoint để lấy danh sách vùng phát hiện hiện tại
@app.route('/get_person_zones', methods=['GET'])
@app.route('/get_aruco_markers', methods=['GET'])
def get_person_zones():
...
@app.route('/clear_person_zones', methods=['POST'])
@app.route('/clear_aruco_markers', methods=['POST'])
def clear_person_zones():
    global aruco_markers
    aruco_markers = {}
    socketio.emit('person_zones_update', {'zones': aruco_markers})
    socketio.emit('aruco_markers_update', {'markers': aruco_markers})
